        ...
        <div class="form-actions" style="text-align:right; margin-bottom: 20px">
            <?php
                ...
                if(empty($keywordGroup)) {    
                    $linkText='Group Leaves';
                    $getParam='index?group=1';
                } else {
                    $linkText='Single Leaves';
                    $getParam='index';
                } 
                ?>
                 <?php
                ...
                if($roleId !=4) { 
                    echo $this->Html->link(__($linkText), ['action' => $getParam], 
                                                          ['class' => 'btn btn-secondary',
                                                           'id'=>'toggleView','style'=>'margin-right: 5px;']);
                } 
                    ...
                    <table class="table table-hover">
                        ...
                        <?php foreach ($leaveRequests as $leaveRequest):?>
                            <tr>
                                ...
                                <td><?= $leaveType;?></td>
                                <?php 
                                 $max_date=       ($keywordGroup==null ) ?   $leaveRequest->date_to : $leaveRequest->max_date;
                                 $min_date=       ($keywordGroup==null ) ?   $leaveRequest->date_from : $leaveRequest->min_date;
                                 $number_of_days= ($keywordGroup==null ) ?   $leaveRequest->no_of_days :$leaveRequest->number_of_days;
                                ?>
                                <td><?= date('m/d/Y',strtotime(h( $min_date))); ?></td>
                                <td><?= date('m/d/Y',strtotime(h($max_date)));?></td>
                                <td><?= date('m/d/Y',strtotime(h($leaveRequest->modified)));?></td>
                                <td><?= $this->Number->format($number_of_days) ?></td>
                                <td><?php
                                    ...
                                    ?></td>
                                     <td><?= $leaveRequest->reliever_name;?></td>
                                ...
                                <td class="actions">
                                    <?php 
                                       ...
                                       $id= ($keywordGroup==null ) ? $leaveRequest->id:$leaveRequest->group_id;
                                            ...
                                            if(($nowTime === $leaveRequestDate) || ($nowTime < $leaveRequestDate))
                                            {
                                                if($leaveRequest->is_approved == 0 ||$leaveRequest->is_approved == 1) {
                                                    echo $this->Html->link(
                                                        ...
                                                        ['action' => 'employeesLeaveCancellation', $id],
                                                        [
                                                            'escape' => false,
                                                            'data-toggle' => 'tooltip',
