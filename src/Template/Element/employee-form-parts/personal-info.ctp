<div class="card">
    ...
    <div id="personal-info-data" class="collapse show" role="tabpanel" aria-labelledby="personal-info" data-parent="#accordion">
        <div class="card-body">
            ...
            $role = array(1,2);
                if (in_array($loggedUser['role_id'], $role)) {     
                    if(empty($employeeSalary['salary'])){
                        echo $this->Html->link(__('Add Salary'),
                            ['controller'=>'salary','action' => 'add',$employee->id], ['class' => 'btn btn-primary']
                            ); 
                    } else {
                        echo $this->Html->link(__('Update Salary'),
                            ['controller'=>'salary','action' => 'edit',$employeeSalary['id'],$employee->id], ['class' => 'btn btn-primary']
                            );
//                        echo $this->Html->link(__('Update Salary'),
//                            ['controller'=>'salary','action' => 'add',$employee->id], ['class' => 'btn btn-primary']
//                        );
            ...
            <?php
                    if($isLeaveAdded == 0){
                        echo $this->Html->link(__('Add Total Leave'),
                            ['controller'=>'LeaveDays','action' => 'add', $employee->id,$employeeName,$employee->office_location],['class' => 'btn btn-primary']
                        );
                    } else {
                        echo $this->Html->link(__('Update Total Leave'),
                            ['controller'=>'LeaveDays','action' => 'edit',$leaveDays['id'],$employeeName,$employee['office_location']],['class' => 'btn btn-primary']
                        );
                    }
                }
            ?>
